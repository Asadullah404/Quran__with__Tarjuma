...
export function VisualizerContainer({ 
  ...
  const [currentMode, setCurrentMode] = useState<VisualizerMode>(VISUALIZER_MODES[0]);
  ...
  const renderVisualizer = () => {
    ...
    switch (currentMode.id) {
      ...
      case 'dunes':
        return <div className="w-full h-full bg-gradient-to-b from-yellow-200 to-yellow-600 dark:from-yellow-800 dark:to-yellow-900 flex items-center justify-center">
          <p className="text-foreground text-lg">Desert Dunes - Coming Soon</p>
        </div>;
      case 'calligraphy':
        return <div className="w-full h-full bg-gradient-to-b from-primary/10 to-accent/20 flex items-center justify-center">
          <p className="text-foreground text-lg">Calligraphy - Coming Soon</p>
        </div>;
      case 'particles':
        return <div className="w-full h-full bg-gradient-to-b from-background to-primary/5 flex items-center justify-center">
                ...
                {VISUALIZER_MODES.map((mode) => (
                  <div
                    ...
                    className={cn(
                      ...
                      currentMode.id === mode.id && "bg-primary/20 ring-1 ring-primary"
                    )}
                    onClick={() => {
                      setCurrentMode(mode);
